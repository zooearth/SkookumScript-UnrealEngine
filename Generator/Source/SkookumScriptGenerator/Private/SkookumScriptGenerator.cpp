...
FString FSkookumScriptGenerator::generate_property_getter_binding_code(UProperty * property_p, const FString & class_name_cpp, UStruct * class_or_struct_p, const MethodBinding & binding)
  ...
  // There currently seems to be no combination of flags here that properly allows deciding between direct call and copy call
  // Soooo - this feature is off until UE4 provides the proper flags
  if (false
   && property_p->HasAnyFlags(RF_Public) 
   && !property_p->HasAnyPropertyFlags(CPF_Protected)
   && property_p->HasAnyPropertyFlags(CPF_Edit))
    { // Direct call
    generated_code += TEXT("    if (this_p)\r\n      {\r\n");
    generated_code += FString::Printf(TEXT("      %s\r\n"), *generate_return_value_passing(property_p, FString::Printf(TEXT("this_p->%s"), *property_p->GetNameCPP())));
    generated_code += TEXT("      }\r\n");
    }
  else
    { // Call via CopyCompleteValue()
