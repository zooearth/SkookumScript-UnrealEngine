...
struct SK_API SkNamedIndexed : ANamed
  ...
  // Common Methods
    ...
    SkNamedIndexed() : m_data_idx(0), m_ptr_id(uint16_t(AIdPtr<SkNamedIndexed>::get_next_ptr_id() & 0xffff) | 0x8000) {} // or with 0x8000 to make sure it will _never_ match AIdPtr_null
    SkNamedIndexed(const ASymbol & name, int32_t data_idx) : ANamed(name), m_data_idx((int16_t)data_idx), m_ptr_id(uint16_t(AIdPtr<SkNamedIndexed>::get_next_ptr_id() & 0xffff) | 0x8000) {}
