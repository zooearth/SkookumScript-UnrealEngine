...
// CRC table implementing the polynomial 0x04c11db7
// as used by zip, zlib, zmodem, Python, PHP, MySQL
static const uint32_t s_table_crc32[256] =
  {
...
// CRC table for improved CRC64 algorithm as proposed by David T. Jones
// http://www0.cs.ucl.ac.uk/staff/D.Jones/crcnote.pdf
// Implementing the polynomial 0xad93d23594c935a9
static const uint64_t s_table_crc64[256] =
  {
...
// Intellectual exercise: Verify the CRC tables for correctness
#ifdef A_EXTRA_CHECK
static const struct ACRCTableVerify
  {
  ACRCTableVerify()
    {
    // Verify CRC-32 table implementing the polynomial 0x04c11db7
    for (uint32_t i = 0; i < 256; ++i)
      ...
      for (uint32_t j = 0; j < 8; ++j)
        ...
        if (part & 1)
          {
          part = (part >> 1) ^ 0xedb88320u; // The _reversed_ polynomial
          }
        else
          {
          part >>= 1;
      ...
      A_ASSERTX(s_table_crc32[i] == part, "Incorrect CRC table value.");
      }

    // Verify CRC-64 table implementing the polynomial 0xad93d23594c935a9
    for (uint32_t i = 0; i < 256; ++i)
      {
      uint64_t part = i;
      for (uint32_t j = 0; j < 8; ++j)
        ...
        if (part & 1)
          {
          part = (part >> 1) ^ UINT64_C(0x95AC9329AC4BC9B5); // The _reversed_ polynomial
          }
        else
          {
          part >>= 1;
      ...
      A_ASSERTX(s_table_crc64[i] == part, "Incorrect CRC table value.");
      }
    }
  } s_table_verify;
#endif
...
uint32_t AChecksum::generate_crc32(
  ...
  uint8_t * byte_end_p = byte_p + data_num_bytes;
...
uint32_t AChecksum::generate_crc32_upper(
  ...
  while (byte_p < byte_end_p)
    {
    crc = s_table_crc32[(crc ^ uint32_t(AString::ms_char2uppper[*byte_p++])) & 0xff] ^ (crc >> 8);
    }
...
//---------------------------------------------------------------------------------------
// Creates a checksum via the improved CRC64 algorithm as proposed by David T. Jones
// http://www0.cs.ucl.ac.uk/staff/D.Jones/crcnote.pdf
// Implementing the polynomial 0xad93d23594c935a9
// This can be called successively with previous CRC checksum values.
// Returns: Checksum created via the CRC algorithm.
uint64_t AChecksum::generate_crc64(const void * data_p, uint32_t data_num_bytes, uint64_t prev_crc /*= 0*/)
  {
  ...
  while (byte_p < byte_end_p)
    {
    crc = s_table_crc64[(crc ^ uint64_t(*byte_p++)) & 0xff] ^ (crc >> 8);
    }
...
uint64_t AChecksum::generate_crc64(const AString & str, uint64_t prev_crc /*= 0*/)
  {
  return generate_crc64(str.as_cstr(), str.get_length(), prev_crc);
  }

//---------------------------------------------------------------------------------------

uint64_t AChecksum::generate_crc64_cstr(const char * cstr_p, uint32_t length /*= ALength_calculate*/, uint64_t prev_crc /*= 0*/)
  {
  return generate_crc64(cstr_p, (length == ALength_calculate) ? uint32_t(::strlen(cstr_p)) : length, prev_crc);
  }
